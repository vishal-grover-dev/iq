...
export async function POST(req: NextRequest) {
  try {
    ...
    const body = (await req.json().catch(() => ({}))) as TMcqSaveRequest;
    ...
    if (error) {
      const msg = ((error as unknown as Record<string, unknown>)?.message as string | undefined)?.toLowerCase?.() || "";
      ...
      return NextResponse.json(
        { ok: false, message: (error as unknown as Record<string, unknown>)?.message },
        { status: 400 }
      );
    }
