    ...
    >
      <div className='mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between'>
        <div>
          <h2 className='flex items-center gap-2 text-lg font-semibold md:text-xl'>
            <FunnelSimpleIcon className='h-5 w-5 text-primary/80' weight='fill' />{" "}
            {QUESTION_REVIEW_LABELS.SECTION_TITLE}
          </h2>
        </div>
      ...
      <div className='flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-2'>
        <div className='text-muted-foreground text-sm'>
          {QUESTION_REVIEW_LABELS.FILTERED_SUMMARY_TEMPLATE.replace(
            "{shown}",
        ...
        <div className='text-muted-foreground text-xs uppercase tracking-wide'>
          {QUESTION_REVIEW_LABELS.TIP_MESSAGE}
        </div>
      </div>
            ...
            {groupMode !== "none" && (
              <div className='mb-4 flex items-center justify-between'>
                ...
                <div className='text-muted-foreground text-xs'>
                  {QUESTION_REVIEW_LABELS.GROUP_STATS_TEMPLATE.replace(
                    "{incorrect}",
                  ...
                  >
                    <div className='mb-3 flex items-center gap-2'>
                      <span className='text-muted-foreground text-sm font-medium'>
                        {`${QUESTION_REVIEW_LABELS.QUESTION_PREFIX} ${question.question_order}`}
                      </span>
                      {question.is_correct ? (
                        <div className='flex items-center gap-1 text-sm text-green-600 dark:text-green-400'>
                          <CheckCircleIcon weight='fill' className='h-4 w-4' />
                          <span>{QUESTION_REVIEW_LABELS.CORRECT_LABEL}</span>
                        </div>
                      ) : (
                        <div className='flex items-center gap-1 text-sm text-red-600 dark:text-red-400'>
                          <XCircleIcon weight='fill' className='h-4 w-4' />
                          <span>{QUESTION_REVIEW_LABELS.INCORRECT_LABEL}</span>
                        </div>
                      )}
                    </div>
